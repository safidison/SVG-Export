...
function generateSVGList(action, imgData) {
    var iconImage = (imgType === "jpeg") ? imgData.jpegURL : imgData.pngURL,
        imageName = (imgType === "jpeg") ? imgData.title + '.jpg' : imgData.title + '.' + imgType,
        hrefVal;
...
function generateSVGData(callbackAction, callback) {
    ...
    image.onerror = (err) => {
        convertionQueue.shift();
        generateSVGData(callbackAction, callback);
        console.log('There seems to be an error loading image ' + currentData.title);
        throw err;
    }
}
...
function displayReceviedSVGs(receivedData) {
    ...
    originalSVGData.forEach((data, i) => { data.id = i });
...
function changeOutputType(event) {
    ...
    document.getElementById('settings').setAttribute('class', imgType);

    imgTypeButtons.forEach(button => button.setAttribute('class', ''));
    event.target.setAttribute('class', 'active');

    svgData.forEach(data => generateSVGList("update", data));
}
...
function downloadSVGs() {
    if (convertionQueue.length > 0) {
        ...
        svgData.forEach((data) => {
            if (document.getElementById('image-checkbox-' + data.id).checked) {
                document.getElementById('image-' + data.id).querySelectorAll('a.download-link')[0].click();
                sendDownloadEvent();
            }
        });
    }
...
window.addEventListener('load', function (evt) {
    imgTypeButtons.forEach(button => button.addEventListener('click', changeOutputType));
